#!/usr/bin/env python3
"""
Atlan Data Extractor - Test Configuration Version

This version allows testing with different configuration files.
Usage: python main_test.py [config_file]
Example: python main_test.py configs/config_test.json
"""

import sys
import os

# Add the ability to specify config file as command line argument
...
logger.info(f"Starting multi-subdomain Atlan data extraction with config: {config_file}")
logger.info(f"Found {len(SUBDOMAIN_AUTH_MAP)} subdomains to process: {list(SUBDOMAIN_AUTH_MAP.keys())}")


def cleanup_old_files():
    """Remove log files and output files older than 30 days."""
    files_deleted = 0
                    ...
                    for db in connection_databases:
                        combined_row = {
                            ...
                            'category': 'lake' if connection.get('connector_name') == 'databricks' else 
                                       ...
                                       'visualization' if connection.get('connector_name') in ['tableau', 'powerbi'] else '',
